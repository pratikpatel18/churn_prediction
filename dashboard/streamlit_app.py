...
if page == "📊 Overview":
    ...
    if mlflow_runs is not None and not mlflow_runs.empty:
        if "metrics.f1" in mlflow_runs.columns:
            ...
            best_auc = mlflow_runs["metrics.roc_auc"].max() if "metrics.roc_auc" in mlflow_runs.columns else None
    ...
    if features_df is not None:
        ...
        key_cols = [c for c in ["tenure", "MonthlyCharges", "TotalCharges", "risk_score"]
                    if c in num_cols][:4]
        ...
        if key_cols:
            ...
            for i, col in enumerate(key_cols, 1):
                fig.add_trace(
                    go.Histogram(x=features_df[col], name=col,
        ...
        if "Contract" in features_df.columns and "Churn" in features_df.columns:
            ...
            fig2 = px.bar(grp, x="Contract", y="Churn Rate",
...
# ─────────────────────────────────────────────────────────────────────
    ...
    if drift_report is None:
        ...
        col2.metric("Drifted Features", f"{ds.get('n_drifted_features', 0)} / {ds.get('n_features', 0)}")
        ...
        if reference_df is not None and features_df is not None:
            ...
            num_cols = [c for c in reference_df.select_dtypes(include="number").columns
                        if c in features_df.columns and c != "Churn"][:6]

            if num_cols:
                selected = st.selectbox("Select feature", num_cols)
                fig = go.Figure()
                fig.add_trace(go.Histogram(
                    x=reference_df[selected], name="Reference",
                    opacity=0.6, marker_color="#6366f1",
                    histnorm="probability density",
                ))
                fig.add_trace(go.Histogram(
                    x=features_df[selected], name="Current",
...
# ─────────────────────────────────────────────────────────────────────
    ...
    if mlflow_runs is None or mlflow_runs.empty:
        ...
        display_cols = (
            ...
            + [c for c in metric_cols if any(m in c for m in ["f1", "accuracy", "roc_auc", "recall"])]
            + ["start_time"]
        )
        ...
        disp.columns = [c.replace("metrics.", "").replace("tags.", "").replace("params.", "")
            ...
            if resp.status_code == 200:
                ...
                fig = go.Figure(go.Indicator(
                    ...
                    gauge = {
                        ...
                        "threshold": {"value": 50, "line": {"color": "black", "width": 3}},
                    },
                ))
