...
class FeatureEngineer:
    ...
    def load_preprocessor(self, path=None):
        for p in [path, str(self.artifact_dir / "preprocessor.joblib"), "artifacts/models/preprocessor.joblib", "/app/artifacts/models/preprocessor.joblib"]:
            if p is None:
                continue
            try:
    ...
    def _create_features(self, df):
        ...
        df["risk_score"] = df["high_risk_contract"] * 3 + df["fiber_optic"] * 2 + df["electronic_check"] * 1 + (df["tenure"] < 12).astype(int) * 2
