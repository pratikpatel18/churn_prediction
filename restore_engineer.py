...
class FeatureEngineer:
    ...
    def fit_transform(self, df):
        ...
        logger.success(f"fit_transform complete")
        return df
    ...
    def load_preprocessor(self, path=None):
        for p in [
            ...
            try:
                ...
                scaler_p = str(p).replace("preprocessor", "scaler")
                try:
                ...
                continue
        logger.warning("Could not load preprocessor")
    ...
    def _create_features(self, df):
        ...
        df["risk_score"] = (
            df["high_risk_contract"] * 3 + df["fiber_optic"] * 2 +
            df["electronic_check"] * 1 + (df["tenure"] < 12).astype(int) * 2
        )
    ...
    def _encode_and_scale(self, df, fit=True):
        ...
        if fit:
            ...
            scaler = None
            for sp in [str(self.artifact_dir / "scaler.joblib"), "artifacts/models/scaler.joblib", "/app/artifacts/models/scaler.joblib"]:
                try:
...
with open("/app/src/features/engineer.py", "w") as f:
    f.write(content)
print("engineer.py restored successfully")
